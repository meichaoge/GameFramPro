...
/// <summary>
/// RectTransform 扩展类 (有些方法可以使用RectTransformUtility)
/// </summary>
public static class RectTransform_Ex
{
    ...
    /// <param name="c"></param>
    /// <returns></returns>
    public static Rect GetCanvasRect(this RectTransform trans, Canvas c)
    {
        if (c == null)
            return new Rect();

        trans.GetWorldCorners(m_WorldCorners);
        var canvasTransform = c.transform;
        for (int i = 0; i < 4; ++i)
    ...
    /// <summary>
    /// 获取相对于relativeTrans 的坐标范围
    /// </summary>
    /// <param name="trans"></param>
    /// <param name="relativeTrans"></param>
    /// <param name="c"></param>
    /// <returns></returns>
