...
namespace GameFramePro.NetWorkEx
{
    ...
    public class AssetBundleDownloadManager : UnityWebRequestDownLoadManager
    {
        ...
        public virtual void GetDataFromUrl(string taskUrl, uint crc, Action<UnityWebRequest, bool, string> callback, UnityTaskPriorityEnum priorityEnum = UnityTaskPriorityEnum.Normal)
        {
            ...
            while (targetNode != null)
            {
                ...
                if (targetNodeValue.TaskPriorityEnum < priorityEnum)
                {
                    newDownLoadTask = GetDownloadTaskInstance(taskUrl, crc, callback, priorityEnum);
        ...
        protected UnityWebRequestDownloadTask GetDownloadTaskInstance(string taskUrl, uint crc, Action<UnityWebRequest, bool, string> callback, UnityTaskPriorityEnum priorityEnum)
        {
            var newTask = mUnityWebRequestTaskPoolManager.GetItemFromPool();
            UnityWebRequest webRequest = new UnityWebRequest(taskUrl);
            webRequest.downloadHandler = new DownloadHandlerAssetBundle(taskUrl, crc);
