...
public  static partial class ZipUtility
{
    ...
    public static bool Zip(string[] _fileOrDirectoryArray, string _outputPathName, string _password = null, ZipCallback _zipCallback = null)
    {
        ...
        foreach (var fileOrDirectory in _fileOrDirectoryArray)
        {
            bool result = false;
...
//TODO  还没有完成
public static  class ZipUtility1
{
    ...
    /// <param name="_zipCallback">ZipCallback对象，负责回调</param>
    /// <returns></returns>
    public static async Task<bool> ZipPro(string[] _fileOrDirectoryArray, string _outputPathName, string _password = null, ZipUtility.ZipCallback _zipCallback = null)
    {
        if ((null == _fileOrDirectoryArray) || string.IsNullOrEmpty(_outputPathName))
        {
            if (null != _zipCallback)
        ...
        foreach (var fileOrDirectory in _fileOrDirectoryArray)
        {
            await ZipFileOrDictoryAsync(fileOrDirectory, zipOutputStream, _zipCallback);
        }
    ...
    private static async Task<bool> ZipFileOrDictoryAsync(string fileOrDirectory, ZipOutputStream zipOutputStream, ZipUtility.ZipCallback zipCallback)
    {
        bool result = false;

        if (Directory.Exists(fileOrDirectory))
            result = await ZipDirectoryPro(fileOrDirectory, string.Empty, zipOutputStream, zipCallback);
        else if (File.Exists(fileOrDirectory))
            result = await ZipFilePro(fileOrDirectory, string.Empty, zipOutputStream, zipCallback);
    ...
    /// <param name="_unzipCallback">UnzipCallback对象，负责回调</param>
    /// <returns></returns>
    public static async Task<bool> UnzipFilePro(string _filePathName, string _outputPath, string _password = null, ZipUtility.UnzipCallback _unzipCallback = null)
    {
        if (string.IsNullOrEmpty(_filePathName) || string.IsNullOrEmpty(_outputPath))
        {
            if (null != _unzipCallback)
        ...
        try
        {
            return await  UnzipFilePro(File.OpenRead(_filePathName), _outputPath, _password, _unzipCallback);
        }
        catch (System.Exception _e)
        {
            Debug.LogError("[ZipUtility.UnzipFile]: " + _e.ToString());
    ...
    /// <param name="_unzipCallback">UnzipCallback对象，负责回调</param>
    /// <returns></returns>
    public static async Task<bool> UnzipFilePro(byte[] _fileBytes, string _outputPath, string _password = null, ZipUtility.UnzipCallback _unzipCallback = null)
    {
        if ((null == _fileBytes) || string.IsNullOrEmpty(_outputPath))
        {
            if (null != _unzipCallback)
        ...
        bool result  = await UnzipFilePro(new MemoryStream(_fileBytes), _outputPath, _password, _unzipCallback);
        if (!result)
        {
            if (null != _unzipCallback)
                _unzipCallback.OnFinished(false);
    ...
    /// <param name="_unzipCallback">UnzipCallback对象，负责回调</param>
    /// <returns></returns>
    public  static async Task<bool> UnzipFilePro(Stream _inputStream, string _outputPath, string _password = null, ZipUtility.UnzipCallback _unzipCallback = null)
    {
        if ((null == _inputStream) || string.IsNullOrEmpty(_outputPath))
        {
            if (null != _unzipCallback)
    ...
    /// <param name="_zipCallback">ZipCallback对象，负责回调</param>
    /// <returns></returns>
    private  static async Task<bool> ZipFilePro(string _filePathName, string _parentRelPath, ZipOutputStream _zipOutputStream, ZipUtility.ZipCallback _zipCallback = null)
    {
        //Crc32 crc32 = new Crc32();
        ZipEntry entry = null;
        FileStream fileStream = null;
    ...
    /// <param name="_zipCallback">ZipCallback对象，负责回调</param>
    /// <returns></returns>
    private  static async Task<bool> ZipDirectoryPro(string _path, string _parentRelPath, ZipOutputStream _zipOutputStream, ZipUtility.ZipCallback _zipCallback = null)
    {
        ZipEntry entry = null;
        try
        {
            ...
            string[] files = Directory.GetFiles(_path);
            for (int index = 0; index < files.Length; ++index)
              await  ZipFilePro(files[index], Path.Combine(_parentRelPath, Path.GetFileName(_path)), _zipOutputStream, _zipCallback);
        }
        catch (System.Exception _e)
        {
            Debug.LogError("[ZipUtility.ZipDirectory]: " + _e.ToString());
        ...
        for (int index = 0; index < directories.Length; ++index)
        {
            bool isSuccess = await ZipDirectoryPro(directories[index], Path.Combine(_parentRelPath, Path.GetFileName(_path)), _zipOutputStream, _zipCallback);
                if (isSuccess == false)
                return false;
         //   if (!ZipDirectoryPro(directories[index], Path.Combine(_parentRelPath, Path.GetFileName(_path)), _zipOutputStream, _zipCallback))
           // return false;
        }
