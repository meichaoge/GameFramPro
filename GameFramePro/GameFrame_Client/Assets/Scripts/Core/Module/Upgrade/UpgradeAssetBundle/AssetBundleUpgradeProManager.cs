...
namespace GameFramePro.Upgrade
{
    ...
    ///  5 解压覆盖本地版本
    /// </summary>
    public class AssetBundleUpgradeProManager : Single<AssetBundleUpgradeProManager>//, IUpgradeModule
    {
        #region 路径配置
        ...
        /// </summary>
        private string mServerAssetBundleConfigUrl { get { return ApplicationManager.S_TopCDNUrl.CombinePathEx(ConstDefine.S_AssetBundleConfigFileName); } }
        private string mLocalAssetBundleConfigUrl { get { return ConstDefine.S_LoadDownloadAssetStoryDirectoryUri.CombinePathEx(ConstDefine.S_AssetBundleConfigFileName); } }


        /// <summary>
        /// AssetBundle下载完成前资源被下载在这里 然后解压和覆盖原来的资源
        /// </summary>
        private static string _AssetBundleDownloadTempDirectory = string.Empty;
        private static string s_AssetBundleDownloadTempDirectory
        {
            get
            {
                if (string.IsNullOrEmpty(_AssetBundleDownloadTempDirectory))
                    _AssetBundleDownloadTempDirectory = $"{Application.persistentDataPath}/{ConstDefine.S_LocalStoreDirectoryTempName}/{ConstDefine.S_AssetBundleDirectoryName}";
                return _AssetBundleDownloadTempDirectory;
            }
        }
        #endregion
        ...
        private AssetBundleAssetTotalInfor mLocalAssetBundleAssetTotalInfor = null; //从服务器获取的 AssetBundle 配置
                                                                                    //key =AssetBundleName
        ...
        public void OnUpgradeSuccess()
        {
            ...
            IOUtility.CreateOrSetFileContent(mLocalAssetBundleConfigUrl, mServerBundleAssetConfigStr);
        ...
        {
            #region 获取本地的资源版本信息
            if (System.IO.File.Exists(mLocalAssetBundleConfigUrl))
            {
                IOUtility.GetFileContent(mLocalAssetBundleConfigUrl, out var localAssetBundleConfig);
        ...
        private IEnumerator GetServerAssetBundleServerAssetConfig()
        {
            ...
            {
                //var downloadTask = DownloadManager.S_Instance.GetByteDataFromUrl(mServerAssetBundleConfigUrl, TaskPriorityEnum.Immediately, null);
        ...
        private void SaveAssetBundleFromDownload(DownloadHandlerBuffer handle, string assetBundleName)
        {
            ...
            string fileSavePath = s_AssetBundleDownloadTempDirectory.CombinePathEx(assetBundleName);  //先保存在缓存目录
        ...
        public async Task UnZipAssetBundleAndMoveFiles()
        {
            DeleteAllInvalidAssetBundleAsset(mAllNeedDeleteABundleFullUris);
#if UNITY_EDITOR
            ShowAllNeedUpdateAssetBundleByType(mAllNeedUpdateAssetBundleAssetInfor);
#endif
            mAllNeedDeleteABundleFullUris.Clear();
            await Task.Delay(TimeSpan.FromSeconds(0.1f));

            await UnZipAssetBundlePackage();
        }
        ...
        /// </summary>
        /// <returns></returns>
        private async Task UnZipAssetBundlePackage()
        {
            await Task.Delay(TimeSpan.FromSeconds(1f));
            Debug.LogInfor($"TODO  目前没有实现");
        }

        /// <summary>
        /// 移动所有的资源覆盖原有的资源
        /// </summary>
        private void MoveAssetBundleFiles()
        {
        }
