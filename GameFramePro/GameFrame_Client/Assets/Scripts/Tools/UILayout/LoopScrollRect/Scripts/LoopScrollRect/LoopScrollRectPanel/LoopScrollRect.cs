...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
...
#if UNITY_EDITOR
        //****测试发现 ItemTypeStart 和 ItemTypeEnd 就是代表当前显示区域项的开始索引和结束缩影，不过
        //需要注意的是 ItemTypeEnd 比实际的多一个

        [Header("编辑器下显示循环列表上最上 /左面元素索引")]
        [SerializeField]
        private int ItemTypeStart_EditorShow;
        [Header("编辑器下显示循环列表上最下/右面元素索引")]
        [SerializeField]
        private int ItemTypeEnd_EditorShow;

        private void Update()
        {
            ItemTypeStart_EditorShow = ItemTypeStart;
            ItemTypeEnd_EditorShow = ItemTypeEnd;
        }
#endif
        ...
        public void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                for (int i = 0; i < content.childCount; i++)
                {
                    ...
                    {
                        content.GetChild(i).GetComponent<LoopScrollRectItemBase>().InitialedScrollCellItem(ItemTypeEnd);
                        if (m_OnItemViewFlushEvent != null)
                            m_OnItemViewFlushEvent(ItemTypeEnd);  //刷新数据视图
