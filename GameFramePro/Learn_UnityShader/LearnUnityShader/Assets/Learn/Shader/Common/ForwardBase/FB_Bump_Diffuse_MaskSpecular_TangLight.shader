...
Shader "Unlit/Common/FB_Bump_Diffuse_MaskSpecular_TangLight"
{
//适用于前向渲染中主光源关照  切线空间计算光照 通关遮罩纹理控制高光
//普通纹理+法线纹理+遮罩纹理 ForwardBase 光照 ，漫反射+环境光 +高光
//原理 通过遮罩纹理中某个或者几个通道的值然后混合到某个计算上
    Properties
    {
		...
		_SpecularMaskScale("Specular Mask Scale",Range(0,2.0))=1.0
    }
    SubShader
    {
	   ...
	   #include "Lighting.cginc"
		 ...
		 struct v2f
            ...
            fixed4 frag (v2f i) : SV_Target
            {
				...
				fixed4 specularMaskValue=tex2D(_SpecularMask,i.uv.zw);

				fixed3 halfDir=normalize(tangentLightDir+tangentViewDir);
				fixed3 specularColor= _LightColor0.rgb*_Specular* pow(saturate(dot(halfDir,tangetNormal)),_Gloss)*specularMaskValue.a*_SpecularMaskScale;
