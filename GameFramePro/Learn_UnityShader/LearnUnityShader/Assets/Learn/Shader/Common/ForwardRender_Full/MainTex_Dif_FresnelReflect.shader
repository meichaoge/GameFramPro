...
Shader "Unlit/Common/MainTex_Dif_FresnelReflect"
{
//适用于使用前向渲染且处理各种逐像素的光源  +反射立方体纹理  +菲涅耳反射
//采用两个Pass 第一个ForwardBase 第二个ForwardAdd
	...
	//	_ReflectAmount("Reflect Amout",Range(0,1.0))=0.5 //折射比例 用于混合颜色
		_FresnelScale("Fresnel ",Range(0,1.0))=0.5  //菲涅耳系数
		...
		// 聚光灯的_LightTexture0存储的不再是基于距离的衰减纹理，而是一张基于张角范围的衰减纹理
		 ...
		 atten = 1.0;
            ...
            fixed4 frag_Forwardbase (v2f i) : SV_Target
            {
				...
				fixed fresnel=_FresnelScale+(1-_FresnelScale)*   pow(1-dot(worldNormalDir,worldViewDir),5);                     

				fixed3 diffuse_Reflect=lerp(diffuseColor,reflectColor,fresnel);
